...
result <- foreach(i = 1:n_rep_, .packages = c("dbarts","SoftBart","MOTRbart","dplyr")) %dopar%{
  ...
  devtools::load_all()
  source("/users/research/mmarques/spline_bart_lab/rspBART7/R/sim_functions.R")
  source("/users/research/mmarques/spline_bart_lab/rspBART7/R/main_function.R")
  source("/users/research/mmarques/spline_bart_lab/rspBART7/R/cv_functions.R")
...
# Saving all
# if(all_){
# saveRDS(object = result,file = paste0("/localusers/research/mmarques/spline_bart_lab/preliminar_results/rspBART7/oned_n_",n_,
...
if(type_ == "friedman_nointer_nonoise"){
    if(all_){
      if(!stump_){
          saveRDS(object = result,file = paste0("/users/research/mmarques/spline_bart_lab/preliminar_results/rspBART7/friedman_noint_nonoise/all_variables/oned_n_",n_,
                                                "_sd_",sd_,"_nIknots_",nIknots_,"_ntree_",ntree_,"_bs_",use_bs_,"_motr_bart_",motr_bart_,"_allvar_",all_,"_stump_",stump_,".Rds"))
      } else {
        saveRDS(object = result,file = paste0("/users/research/mmarques/spline_bart_lab/preliminar_results/rspBART7/friedman_noint_nonoise/stump/oned_n_",n_,
      ...
      saveRDS(object = result,file = paste0("/users/research/mmarques/spline_bart_lab/preliminar_results/rspBART7/friedman_noint_nonoise/oned_n_",n_,
